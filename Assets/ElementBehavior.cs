...
public class ElementBehavior : MonoBehaviour
{
    ...
    IEnumerator SetSpeed()
    {
        ...
        if(machine.isStopped)
        {
            ...
            //Memory.chosen.Add(this);
            
            yield return new WaitUntil(() => transform.position.y == -3);
            FindObjectOfType<Manager>().elements[index] = this;
            index++;
            Memory.isReadyToSet = true;
            machine.isStopped = false;
            StopAllCoroutines();
            //transform.position = Vector2.MoveTowards(transform.position, new Vector2(transform.position.x, -4), Time.deltaTime);
        }
